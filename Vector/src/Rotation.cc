...
CLHEPdouble HepRotation::operator() (int i, int j) const {
  if (i == 0) {
    if (j == 0) { return xx(); }
    if (j == 1) { return xy(); }
    if (j == 2) { return xz(); } 
...
HepRotation & HepRotation::rotate(CLHEPdouble a, const Hep3Vector& aaxis) {
  if (a != 0.0) {
    CLHEPdouble ll = aaxis.mag();
    if (ll == 0.0) {
      ZMthrowC (ZMxpvZeroVector("HepRotation: zero axis"));
    }else{
      CLHEPdouble sa = std::sin(a), ca = std::cos(a);
      CLHEPdouble dx = aaxis.x()/ll, dy = aaxis.y()/ll, dz = aaxis.z()/ll;   
      HepRotation m1(
...
HepRotation & HepRotation::rotateX(CLHEPdouble a) {
  CLHEPdouble c1 = std::cos(a);
  CLHEPdouble s1 = std::sin(a);
  CLHEPdouble x1 = ryx, y1 = ryy, z1 = ryz; 
  ryx = c1*x1 - s1*rzx;
  ryy = c1*y1 - s1*rzy;
  ryz = c1*z1 - s1*rzz;
  rzx = s1*x1 + c1*rzx;
...
HepRotation & HepRotation::rotateY(CLHEPdouble a){
  CLHEPdouble c1 = std::cos(a);
  CLHEPdouble s1 = std::sin(a);
  CLHEPdouble x1 = rzx, y1 = rzy, z1 = rzz; 
  rzx = c1*x1 - s1*rxx;
  rzy = c1*y1 - s1*rxy;
  rzz = c1*z1 - s1*rxz;
  rxx = s1*x1 + c1*rxx;
...
HepRotation & HepRotation::rotateZ(CLHEPdouble a) {
  CLHEPdouble c1 = std::cos(a);
  CLHEPdouble s1 = std::sin(a);
  CLHEPdouble x1 = rxx, y1 = rxy, z1 = rxz; 
  rxx = c1*x1 - s1*ryx;
  rxy = c1*y1 - s1*ryy;
  rxz = c1*z1 - s1*ryz;
  ryx = s1*x1 + c1*ryx;
...
CLHEPdouble HepRotation::phiX() const {
  return (yx() == 0.0 && xx() == 0.0) ? 0.0 : std::atan2(yx(),xx());
}

CLHEPdouble HepRotation::phiY() const {
  return (yy() == 0.0 && xy() == 0.0) ? 0.0 : std::atan2(yy(),xy());
}

CLHEPdouble HepRotation::phiZ() const {
  return (yz() == 0.0 && xz() == 0.0) ? 0.0 : std::atan2(yz(),xz());
}

CLHEPdouble HepRotation::thetaX() const {
  return safe_acos(zx());
}

CLHEPdouble HepRotation::thetaY() const {
  return safe_acos(zy());
}

CLHEPdouble HepRotation::thetaZ() const {
  return safe_acos(zz());
}

void HepRotation::getAngleAxis(CLHEPdouble &angle, Hep3Vector &aaxis) const {
  CLHEPdouble cosa  = 0.5*(xx()+yy()+zz()-1);
  CLHEPdouble cosa1 = 1-cosa;
  if (cosa1 <= 0) {
    angle = 0;
    aaxis  = Hep3Vector(0,0,1);
  }else{
    CLHEPdouble x=0, y=0, z=0;
