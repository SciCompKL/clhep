...
CLHEPdouble Hep3Vector::polarAngle (const Hep3Vector & v2) const {
  return std::fabs(v2.getTheta() - getTheta());
} /* polarAngle */

CLHEPdouble Hep3Vector::polarAngle (const Hep3Vector & v2,
				const Hep3Vector & ref) const {
  return std::fabs( v2.angle(ref) - angle(ref) );
} /* polarAngle (v2, ref) */

// CLHEPdouble Hep3Vector::azimAngle (const Hep3Vector & v2) const 
// is now in the .icc file as deltaPhi(v2)

CLHEPdouble Hep3Vector::azimAngle  (const Hep3Vector & v2,
				const Hep3Vector & ref) const {
  ...
  CLHEPdouble ang = vperp.angle(v2perp);
