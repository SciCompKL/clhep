  ...
  for (n=0; n<8; n++) {
    switch ( xb.b[n] ) {
      ...
      default:
		...
		"Cannot determine byte-ordering of CLHEPdoubles on this system");
    } 
    if (byte_order[n] != UNSET) {
        throw DoubConvException(
		"Confusion in byte-ordering of CLHEPdoubles on this system");
    }    
    byte_order[n] = order;
...
std::vector<unsigned long> DoubConv::dto2longs(CLHEPdouble d) {
...
CLHEPdouble DoubConv::longs2CLHEPdouble (const std::vector<unsigned long> & v) {
  DB8 db;
